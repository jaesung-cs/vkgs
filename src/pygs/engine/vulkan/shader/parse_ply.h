#ifndef PYGS_ENGINE_VULKAN_SHADER_PARSE_PLY_H
#define PYGS_ENGINE_VULKAN_SHADER_PARSE_PLY_H

namespace pygs {
namespace vk {
namespace shader {

const char* parse_ply_comp = R"shader(
#version 460 core
...
layout (std430, set = 1, binding = 4) writeonly buffer GaussianSh {
  float gaussian_sh[];  // (N, 3, 4, 4), 16 values packed with 4 vec4.
};
...
void main() {
  ...
  if (id < point_count) {
    ...
    vec3 s = vec3(ply[base * id + local_offsets[3]], ply[base * id + local_offsets[4]], ply[base * id + local_offsets[5]]);
    s = exp(s);  // activation

    vec4 q = vec4(ply[base * id + local_offsets[6]], ply[base * id + local_offsets[7]], ply[base * id + local_offsets[8]], ply[base * id + local_offsets[9]]);
    q = q / length(q);
    ...
    #pragma unroll
    for (int i = 0; i < 48; ++i) {
      gaussian_sh[48 * id + i] = ply[base * id + local_offsets[10 + i]];
    }
...
#endif  // PYGS_ENGINE_VULKAN_SHADER_PARSE_PLY_H
