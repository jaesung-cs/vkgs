...
class CpuBuffer::Impl {
 public:
  ...
  Impl(Context context, VkDeviceSize size) : context_(context), size_(size) {
    ...
    vmaCreateBuffer(context.allocator(), &buffer_info, &allocation_create_info, &buffer_, &allocation_,
                    &allocation_info);
...
CpuBuffer::CpuBuffer(Context context, VkDeviceSize size) : impl_(std::make_shared<Impl>(context, size)) {}
