...
class ComputePipeline::Impl {
 public:
  ...
  Impl(Context context, const ComputePipelineCreateInfo& create_info) : context_(context) {
    VkShaderModule compute_module;
    VkShaderModuleCreateInfo shader_info = {VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};
    shader_info.codeSize = create_info.source.size();
    shader_info.pCode = create_info.source.data();
    vkCreateShaderModule(context_.device(), &shader_info, NULL, &compute_module);
    ...
    vkCreateComputePipelines(context_.device(), context_.pipeline_cache(), 1, &pipeline_info, NULL, &pipeline_);
...
ComputePipeline::ComputePipeline(Context context, const ComputePipelineCreateInfo& create_info)
